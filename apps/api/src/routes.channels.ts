...
router.patch("/channels/:id", authRequired, requireRole("OPS"), async (req, res) => {
  ...
  if (updates.platformRef !== undefined) {
    values.push(updates.platformRef);
    setClauses.push(`"platformRef" = $${values.length}`);
  }
  if (updates.platformUserRef !== undefined) {
    values.push(updates.platformUserRef);
    setClauses.push(`"platformUserRef" = $${values.length}`);
  }
